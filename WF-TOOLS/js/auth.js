...
(function (global) {
  ...
  async function updateCredits() {
    ...
    const query = supabase
      .from("profiles")
      .select(
        "plan, credits, full_name, display_name, name, contact_name, owner_name"
      )
      .eq("id", userId)
      .limit(1);

    const execute = typeof query.maybeSingle === "function" ? query.maybeSingle : query.single;
    const { data, error } = await execute.call(query);
